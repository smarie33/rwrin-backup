...
function get_breadcrumb($cpt = "") {
  echo '<a class="bold mt-5" href="'.home_url().'" rel="nofollow">Home</a>';
  if($cpt != ""){
    if($cpt == "interview"){
      echo '&nbsp; > <a class="bold" href="'.home_url("/interviews").'" rel="nofollow">Interviews</a> &nbsp; ';
    }
    if($cpt == "documentaries"){
      error_log("documentaries");
      echo '&nbsp; > <a class="bold" href="'.home_url("/thematic-explorations").'" rel="nofollow">Thematic Explorations</a> &nbsp; ';
    }
...
function getEmbedLink($url){
  $thecheck = 'watch?v=';
  if(strpos($url,$thecheck) !== false){
    $urlfrag = explode($thecheck, $url);
    $getCode = $urlfrag[1];
    if(strpos($urlfrag[1], '&')){
      $getCode = explode('&', $urlfrag[1])[0];
      error_log($getCode);
    }
    $url = 'https://www.youtube.com/embed/'.$getCode;
  }
  return $url;
}

function createVideoIframe($url){
  return '<iframe src="'.getEmbedLink($url).'" width="100%" height="auto" frameborder="0"></iframe>';
}

add_action( 'pre_get_posts', 'rwrin_include_custom_post_types_in_search_results' );
function rwrin_include_custom_post_types_in_search_results( $query ) {
    if ( $query->is_main_query() && $query->is_search() && ! is_admin() ) {
        $query->set( 'post_type', array( 'post', 'page', 'interview', 'committee', 'documentaries' ) );
    }
...
function redirect_post_type_single(){
    if ( ! is_singular( 'committee' ) )
        return;
    wp_redirect( get_page_link( 71 ), 301 );
    exit;
}
